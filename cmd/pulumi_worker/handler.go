...
func (nh *NatsHandler) configureWorkspace(stack *auto.Stack, env Environment) error {
	...
	if env == Environment_ENTRA {
		err := workspace.InstallPlugin(nh.ctx, "azuread", "v6.9.1")
		if err != nil {
			return fmt.Errorf("failed to install program plugins: %v\n", err)
		}
		return nil
	}
	err := workspace.InstallPlugin(nh.ctx, "azure-native", "v3.19.0")
	if err != nil {
		return fmt.Errorf("failed to install program plugins: %v\n", err)
	}
	return nil
...
func (nh *NatsHandler) handleEnv(env Environment) error {
	...
	s, err := auto.UpsertStackInlineSource(ctx, stackName, projectName, pulumiRunFunc)
	if err != nil {
	...
	s.SetConfig(ctx, "azure-native:location", auto.ConfigValue{Value: nh.config.Region})
